...
from app.db.schemas import GoodsCategoryFilter, GoodsCategoryModel,GoodsModel,GoodsFilter
            ...
            async with aiohttp.ClientSession() as http_session:
                ...
                if response.status == 200:
                    ...
                    for offer in offer_mappings:
                        ...
                        if not await GoodsDAO.find_one_or_none(session,GoodsFilter(name=name)):
                            ...
                            try:
                                ...
                                continue
                    page_token = data.get('result', {}).get('paging', {}).get('nextPageToken')  # Получение токена следующей
                    # страницы
                    if not page_token:
...
async def add_category(category_id, name):
    try:
        async with async_session_maker() as session:
            ...
            existing_category =  await GoodsCategoryDAO.find_one_or_none(session,GoodsCategoryFilter(category_id=category_id))
